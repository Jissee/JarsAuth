...
public class CAPendingList {
    ...
    private final ArrayList<ServerPlayerEntity> newPlayers = new ArrayList<>();
    ...
    public void allowNew(ServerPlayerEntity player){
        boolean isNew;
        synchronized (newPlayers){
    ...
    private void tick() {
        ...
        synchronized (waiting){
            ...
            for(Object obj : keys){
                ...
                ServerPlayerEntity player = Compatibility.getPlayerByName(name);
        ...
        synchronized (newPlayers){
            ...
            for(int i = 0; i < count; i++){
                ServerPlayerEntity player = newPlayers.get(i);
                if(Compatibility.hasDisconnected(player)){
                    newPlayers.remove(i);
                    i--;
                }
            ...
            synchronized (loggedIn){
                ...
                for(int i = 0; i < count; i++){
                    ...
                    if(millTime - loginTime > interval * 1000 && !waiting.containsKey(playerName)){
                        ...
                        String serverID = Settings.getServerID().getServerID();
                        CABroadcastPacket packet = new CABroadcastPacket(serverID, CABroadcastPacket.PUBLIC_KEY, pair.getPublic().getEncoded());
                        ServerPlayerEntity svplr = Compatibility.getPlayerByName(rec.playerName);
    ...
    public void addAuthInfo(String serverID, ServerPlayerEntity player, byte[] payload) {
        synchronized (waiting){
            try {
    ...
    private static class Record implements Serializable {
        ...
        public static Optional<Record> searchDiskFor(String playerName){
