...
public class FCPendingList {
    ...
    public void addHash2(ServerPlayerEntity player, String hash){
        synchronized (LOCK){
            for(int i = 0; i < records.size(); i++){
                Record r = records.get(i);
                synchronized (r){
            ...
            for (int i = 0; i < records.size(); i++) {
                ...
                synchronized (r){
                    ...
                    if (!r.expected1.equals(NONE)                //服务端完成计算
                        ...
                        ServerPlayerEntity player = r.player;
            ...
            for (Record r : records) {
                synchronized (r) {
                    ...
                    if (r.expected1.equals(NONE)    //服务端未完成计算
                        ...
                        r.time = System.nanoTime();
                        Compatibility.sendVanillaPacket(r.player, new ResourcePackSendS2CPacket(UUID.randomUUID(), str, "JARSAUTH AUTHENTICATION INFORMATI0N", false, Optional.of(Compatibility.literal(salt1))));
                        Compatibility.sendModPacket(r.player, new FCBroadcastPacket(str, "JARSAUTH AUTHENTICATION INFORMATI0N", Optional.of(Compatibility.literal(salt2))));
    ...
    private static class Record {
        ...
        private Record(ServerPlayerEntity player, long time, String expected1, String got1, String expected2, String got2){
            this.player = player;
            this.time = time;
